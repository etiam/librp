...
// Iterate over all roots (the head) and print each one on a new line by calling printSingleRoot.
template<class T>
void
print_tree_bracketed(const tree<T>& t, std::ostream& str)
{
...
template<class T>
void
print_subtree_bracketed(const tree<T>& t, typename tree<T>::iterator iRoot, std::ostream& str)
{
    if(t.empty()) return;
    if (t.number_of_children(iRoot) == 0)
    {
        str << std::string(2*t.depth(iRoot), ' ') << *iRoot;
    }
    else
    {
        // parent
        str << std::string(2*t.depth(iRoot), ' ') << *iRoot << std::endl;
